...
import { getStoredFileIds, removeFileIds, saveFileIds } from "@/utils/fileNameUtils";
...
const FilePreview: React.FC<FilePreviewProps> = ({ filePreview, setFilePreview, conversationId }) => {
	...
	const handleRemoveFile = async (fileName: string) => {
		...
		if (fileToRemove.id) {
			try {
				...
				if (conversationId) {
					const currentIds = getStoredFileIds(FILE_IDS_STORAGE_KEY, conversationId);
					const newIds = currentIds.filter(id => id !== fileToRemove.id) || [];
					if (newIds && newIds?.length > 0)
						saveFileIds(FILE_IDS_STORAGE_KEY, conversationId, newIds);
					if (newIds?.length === 0) removeFileIds(FILE_IDS_STORAGE_KEY, conversationId);
				}
