 ...
 */
export const getBaseName = (filename: string): string => filename.split('/').pop() || filename;
 ...
 */
export const getUniqueFileName = (existingFiles: string[], originalName: string): string => {
  ...
  if (!baseNames.some(name => name.toLowerCase() === originalName.toLowerCase())) {
    return originalName;
  }
  ...
  const baseNamePattern = new RegExp(`^${baseNameWithoutExt}(?:\\((\\d+)\\))?${escapeRegExp(ext)}$`, 'i');
 ...
 */
export const saveFileIds = (storageKey: string, conversationId: string, fileIds: string[]): void => {
 ...
 */
export const getStoredFileIds = (storageKey: string, conversationId: string): string[] => {
  const key = `${storageKey}_${conversationId}`;
 ...
 */
export const removeFileIds = (storageKey: string, conversationId: string): void => {
  const key = `${storageKey}_${conversationId}`;
