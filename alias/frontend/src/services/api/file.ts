import { DeleteFileResponse, FileResponse, ShareFileRequest, UploadFileResponse } from "@/types/api";
...
fileInstance.interceptors.request.use((config) => {
  const token = localStorage.getItem("access_token") || import.meta.env.VITE_API_ACCESS_TOKEN;
  if (token) {
...
export const fileApi = {
  ...
  // Upload file
  uploadFile: (file: File, onUploadProgress?: (progressEvent: any) => void, abortController?: AbortController) => {
  ...
  downloadFileWithName: async (fileId: string, defaultFileName: string = 'downloaded_file') => {
    try {
