...
export const BaseMessage: React.FC<BaseMessageProps> = ({ message, children, onFeedback }) => {
  ...
  return (
    <div className={`${styles.message} ${isAssistant ? styles.assistant : styles.user}`}>
      <div className={styles.content}>
        ...
        {isWaiting && <div className={styles.waiting}>Waiting for user selection...</div>}
        {isError && <div className={styles.error}>An error occurred</div>}
      </div>
      {isAssistant && onFeedback && (
        <div className={styles.feedback}>
          <button
            className={`${styles.feedbackBtn} ${message.feedback === 'like' ? styles.active : ''}`}
            onClick={() => onFeedback(message.id, message.feedback === 'like' ? null : 'like')}
          >
            👍
          </button>
          <button
            className={`${styles.feedbackBtn} ${message.feedback === 'dislike' ? styles.active : ''}`}
