...
6. NEVER use `Read` tool to read PDF file directly.
...
11. When you use the `Write` tool, you **MUST ALWAYS** remember to provide the both the `file_path` and `content` parameters. DO NOT try to use `Write` with long content exceeding 1k tokens at once!!!
