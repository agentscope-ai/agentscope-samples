...
from local_deep_research.loader.splitter import Chunk
from local_deep_research.utils import log
from local_deep_research.vector_db.base import BaseVectorDB, CollectionInfo, RetrievalResult
...
class Milvus(BaseVectorDB):
    ...
    def __init__(
        self,
        default_collection: str = "local_deep_research",
        ...
        **kwargs,
        ...
        Args:
            default_collection (str, optional): Default collection name. Defaults to "local_deep_research".
    ...
    def init_collection(
        self,
        dim: int,
        collection: Optional[str] = "local_deep_research",
        ...
        **kwargs,
        ...
        Args:
            ...
            collection (Optional[str], optional): Collection name. Defaults to "local_deep_research".
    ...
    def _normalize_uri(uri: str) -> str:
        """Normalize uri for milvus lite usage."""
        if uri.startswith("milvus_lite://"):
            path = uri[len("milvus_lite://") :]
            if path and not os.path.exists(path):
                os.makedirs(path, exist_ok=True)
            return uri

        # Treat local paths as milvus lite storage
        if uri.startswith("./") or uri.startswith("/") or uri.endswith(".db"):
            abs_path = os.path.abspath(uri)
            base_dir = os.path.dirname(abs_path) or os.getcwd()
            os.makedirs(base_dir, exist_ok=True)
            os.environ["MILVUS_LITE_DATA_PATH"] = base_dir
    ...
    def clear_db(self, collection: str = "local_deep_research", *args, **kwargs):
        """
        Clear (drop) a collection from the Milvus database.

        Args:
            collection (str, optional): Collection name to drop. Defaults to "local_deep_research".
            *args: Variable length argument list.
            **kwargs: Arbitrary keyword arguments.
        """
        if not collection:
