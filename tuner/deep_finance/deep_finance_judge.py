...
class DeepFinanceJudgeEngine:
    ...
    def _extract_grader_scores(
        ...
        for grader_name, score_list in grader_results.items():
            if score_list and len(score_list) > 0:
                gs = score_list[0]
                scores[grader_name] = (
                    getattr(gs, "score", 0.0) if hasattr(gs, "score") else 0.0
                )
            else:
    ...
    def _fuse_scores(
        ...
        """Fuse scores."""
        contributions = {}
        contributions["rm_contribution"] = (
            self.w.get("finance", 0.0) * finance_score
        )
