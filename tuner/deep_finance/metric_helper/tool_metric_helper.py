...
async def extract_tool_stats_from_agent(
    agent: Any,
    total_time: float = 0.0,  # noqa: W0613  # pylint: disable=unused-argument
) -> Dict[str, Any]:
    """
...
def compute_single_tool_metrics(
    ...
    success_rate = (
        (success_calls / total_calls * 100) if total_calls > 0 else 0.0
    )
    cache_total = cache_hits + cache_misses
    cache_hit_rate = (
        (cache_hits / cache_total * 100) if cache_total > 0 else 0.0
    )
